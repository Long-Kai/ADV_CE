...
"""
Implementation of ADV-SVM
The code is modified from MetaOptNet https://github.com/kjunelee/MetaOptNet/blob/master/models/classification_heads.py
Required package: qpth and cvxpy
"""
...
def VRM_SVM_CS(support, sigma, support_labels, query, n_way, n_shot, gamma=1., C_reg=0.1, double_precision=False, maxIter=15):
    """
    ...
    kernel_matrix = compute_two_vicinal_kernel(support, support, gamma=gamma, sigmaA=sigma, sigmaB=sigma)
    ...
    compatibility = compute_one_vicinal_kernel(query, support, gamma=gamma, sigma=sigma).transpose(-2, -1) ### LK: change to L(xi, x)
...
def compute_one_vicinal_kernel(A, B, gamma, sigma):
    """
    ...
    Parameters:
      A:  a (n_batch, n, d) Tensor.
      B:  a (n_batch, m, d) Tensor. With vicinity normal distribution, variance sigma
      gamma: a (1) Tensor. RBF kernel variance.
      sigma: a (n_batch, m, d) Tensor. variance of vicinity distribution of B
    Returns: a (n_batch, n, m) Tensor.
    """
    ...
    # compute dist matrix
    A_ex = A.unsqueeze(2).repeat(1, 1, B.size(1), 1)
    B_ex = B.unsqueeze(1)
    gamma_and_sigma = gamma**2 + sigma**2
    Sigma_ex = (0.5 / gamma_and_sigma).unsqueeze(1)
    dist_matrix = (((A_ex-B_ex)**2) / Sigma_ex).sum(dim=-1)
    scaler = torch.exp(torch.log(gamma ** 2 / gamma_and_sigma).sum(dim=-1)) ** 0.5

    res = scaler.unsqueeze(1) * torch.exp(-dist_matrix)

    return res


def compute_two_vicinal_kernel(A, B, gamma, sigmaA, sigmaB):
    """
    Constructs a kernel matrix between A and B.
    We assume that each row in A and B represents a d-dimensional feature vector.
    ...
    Parameters:
      ...
      B:  a (n_batch, m, d) Tensor.
      gamma: a (1) Tensor. RBF kernel variance.
      sigmaA: a (n_batch, n, d) Tensor. variance of vicinity distribution of A
      sigmaB: a (n_batch, m, d) Tensor. variance of vicinity distribution of B
    Returns: a (n_batch, n, m) Tensor.
    """
    ...
    A_ex = A.unsqueeze(2).repeat(1, 1, B.size(1), 1)
    B_ex = B.unsqueeze(1)
    sigmaA_ex = sigmaA.unsqueeze(2)
    sigmaB_ex = sigmaB.unsqueeze(1)
    gamma_and_sigma = gamma**2 + sigmaA_ex**2 + sigmaB_ex**2
    Sigma_ex = (0.5 / gamma_and_sigma)
    dist_matrix = (((A_ex-B_ex)**2) / Sigma_ex).sum(dim=-1)
    scaler = torch.exp(torch.log(gamma ** 2 / gamma_and_sigma).sum(dim=-1)) ** 0.5

    res = scaler * torch.exp(-dist_matrix)
